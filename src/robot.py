    ...
    while flag:
        try:
            ...
            text = element.find_element_by_xpath(
                'div[@class="WB_feed_detail clearfix"]/div[@class="WB_detail"]/div[@class="WB_text W_f14"]').strip().text
            print('   微博内容如下：')
            print('        ', text)
            return
...
def operation(driver, elements, last_time):
    ...
    for element in elements:
        ...
        while flag:
            try:
                date = WebDriverWait(element, 10).until(lambda x: x.find_element_by_xpath(
                    'div[@class="WB_feed_detail clearfix"]/div[@class="WB_detail"]/div[2]/a[1]'))
                flag = False
            except:
                print('<----找不到日期标签！---->')
                continue
        ...
        if date > last_time:
            ...
            text_div = element.find_element_by_xpath(
...
def loop(driver, last_time):
    ...
    elements = get_elements(driver,
                            '//div[@id="v6_pl_content_homefeed"]/div[1]/div[@class="WB_feed WB_feed_v3 WB_feed_v4"]/div')
    elements = elements[:-2]
